...
namespace pbrt
{
	...
	public:
		...
		~MemoryArena()
		{
			FreeAligned(currentBlockStartPos);
		...
		void* Alloc(size_t nBytes)
		{
			...
			nBytes = (nBytes + 16 - 1) & ~(16 - 1);
			if (currentBlockPos + nBytes > currentAllocSize)
			{
				// Add current block to _usedBlocks_ list
				if (currentBlockStartPos)
				{
					usedBlocks.push_back(
						std::make_pair(currentAllocSize, currentBlockStartPos));
					currentBlockStartPos = nullptr;
					currentAllocSize = 0;
				}
				     ...
				     iter != availableBlocks.end(); ++iter)
				{
					if (iter->first >= nBytes)
					{
						...
						currentBlockStartPos = iter->second;
				...
				if (!currentBlockStartPos)
				{
					...
					currentBlockStartPos = AllocAligned<uint8_t>(currentAllocSize);
				}
				currentBlockPos = 0;
			}
			void* ret = currentBlockStartPos + currentBlockPos;
	...
	private:
		...
		uint8_t* currentBlockStartPos = nullptr;
